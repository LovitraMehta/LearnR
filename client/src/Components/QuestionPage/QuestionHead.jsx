...
import { toggleSaveQuestion } from '../../actions/question';
...
const QuestionHead = ({ question, notify }) => {
    ...
    const handleSave = () => {
        console.log("Save icon clicked"); // Add for testing
        if (!user) return notify("You must be logged in to save this question.");
        dispatch(toggleSaveQuestion(question._id, user._id));
        notify("Saved/Unsaved successfully!");
    };

    /**
     * Show if user has flashed the question or not
     */
    const upIndex = question.upVote.findIndex((id) => id === String(user._id));
        ...
        <div className='flex flex-col px-4 py-5 rounded-[30px] w-[90%] sm:w-[400px] max-h-[800px] bg-white shadow-lg gap-2'>
            ...
            <div className='options flex justify-between item-center'>
                <div className='flex item-center gap-2'>
                    ...
                    {/* console.log("Rendering save icon for question:", question._id); */}
                    <svg
                        onClick={handleSave}
                        className='saveIt cursor-pointer'
                        width="24"
                        height="24"
                        viewBox="0 0 24 24"
                        fill={isSaved ? "#FFD700" : "none"}  // ✅ Gold when saved
