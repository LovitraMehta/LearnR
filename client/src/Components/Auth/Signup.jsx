...
export default function Signup() {
  ...
  const handleSubmit = (e) => {
    ...
    } else if (!/^[a-zA-Z0-9._%+-]+@(?:institution\.edu|university\.ac\.in|school\.org|srm.edu.in|edu.in|.in)$/i.test(email)) {
      toast.error('Only institutional emails are allowed (e.g., @institution.edu)', {
        position: "top-center",
        autoClose: 5000,
        theme: "light",
      });
    ...
    <div className='w-full h-screen flex flex-col justify-center items-center bg-[#f2f2f2]'>
      ...
      <div className='flex flex-col justify-center items-center px-4 py-5 rounded-[30px] max-w-[300px] sm:max-w-[400px] max-h-[800px] bg-white shadow-lg'>
        <form onSubmit={handleSubmit}>
          ...
          <p className='text-base font-medium mb-4 text-[#727986]'>Enter your details to create your LearnR Account:</p>
          ...
          <div className='flex justify-between gap-1'>
            <div className='flex flex-col mb-4 max-w-[132px] sm:max-w-[200px]'>
              ...
              <input type="text" id='firstName' name='firstName' placeholder='Bhavay' className='border hover:border-[#0090FC] p-2 rounded-[10px] text-sm font-light h-[30px] focus:outline-none' onChange={(e) => { setFirstName(e.target.value) }} />
            </div>

            <div className='flex flex-col mb-4 max-w-[132px] sm:max-w-[200px]'>
              <label htmlFor="lastName" className='text-sm font-semibold mb-2.5'>Last Name*</label>
              <input type="text" id='lastName' name='lastName' placeholder='Kaushal' className='border hover:border-[#0090FC] p-2 rounded-[10px] text-sm font-light h-[30px] focus:outline-none' onChange={(e) => { setLastName(e.target.value) }} />
            </div>
          </div>

          <div className='flex flex-col mb-4'>
            <label className='text-sm font-semibold mb-2.5' htmlFor="email">Email*</label>
            <input className='border hover:border-[#0090FC] p-2 rounded-[10px] text-sm font-light h-[30px] focus:outline-none' type="email" id='email' name='email' placeholder='b.k@outlook.com' onChange={(e) => { setEmail(e.target.value) }} />
          </div>
          ...
          <div className='flex items-center'>
            ...
            <p className='text-xs ml-1'>I agree with LearnR's <Link to="/" className='text-[#0090FC] visited:text-[#0090FC]'>User Agreement</Link> and <Link to="/" className='text-[#0090FC] visited:text-[#0090FC]'>Privacy Policy</Link>.</p>
          </div>
          ...
          <p className='text-xs text-center my-4'>Already have a LearnR account? <Link to="/login" className='text-[#0090FC] visited:text-[#0090FC]'>Log In</Link></p>
        </form>
      </div>
    </div>
  )
