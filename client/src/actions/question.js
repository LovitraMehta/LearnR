...
const toggleSaveQuestion = (questionId, userId) => async (dispatch) => {
  try {
    const { data } = await saveQuestion(questionId, userId);
    dispatch({ type: "TOGGLE_SAVE_QUESTION", payload: data });
  } catch (error) {
    console.log(error);
  }
};
